  ...
  def getPathFromFile(path: String): List[(String, String)] = {
    ...
    val reactionPaths =
      reactions.foldLeft(Nil: List[(NodeSeq, NodeSeq)]) {
        ...
        .flatMap(tp => for (s <- tp._1; p <- tp._2) yield (s, p))
        .map(tp => ((tp._1 \ "@name").toString.dropWhile {
          _ != ':'
        }.tail,
          (tp._2 \ "@name").toString.dropWhile {
            _ != ':'
          }.tail))
    val edges = reactionPaths.map(p => (p._1, p._2))
    edges
...
//    gr.g.values.map(a => a.length).foreach(println)

    gr.intoComponents.map(a => a.length).foreach(println)
//    reactionPath.foreach(println)
  }
}
